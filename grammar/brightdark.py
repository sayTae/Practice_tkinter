...
## 함수 선언 부분 ##
def  loadImage(fname) :
    global XSIZE, YSIZE, fsize, inImage

    inImage=[]
    fsize = os.path.getsize(fname)              # 파일의 크기
    XSIZE = int(math.sqrt(fsize))      # 정방형으로 가정하고 크기 구함
    YSIZE = int(math.sqrt(fsize))
...
if __name__ == "__main__" :
    ...
    window.bind("<Button-1>", Brightly)
    window.bind("<Button-2>", inversion)
    window.bind("<Button-3>", Darkly)
